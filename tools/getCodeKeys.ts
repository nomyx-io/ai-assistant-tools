module.exports = (config: any) => ({
    ...
    function: async (params: any) => (await parseCode(params.path, params.keys, config.openai_api_key)).map((element: any) => element.key),
    exports: {
        parseCode: async (path: string, keys: string | undefined = undefined) => parseCode(path, keys, config.openai_api_key)
    }
});

async function parseCode (path: string, keys: string | undefined = '', apiKey: string) {
    const fs = require('fs');
    const { Assistant } = require("@nomyx/assistant");
    try {
        if(!fs.existsSync(path)) return `Error: File does not exist at path ${path}`;
        let fileContents = fs.readFileSync(path, 'utf8');
        const assistant = await Assistant.create(
            'get_code_keys',
            `You parse source code into a list of key-value pairs. If given a set of keys you parse the source code using those keys.
...
`
        ...
        const code = await assistant.run(fileContents, [], {}, apiKey, (event: string, value: any) => {});
        assistant.delete();
