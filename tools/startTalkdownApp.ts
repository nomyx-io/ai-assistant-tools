...
`
        try {
            ...
            try {
                Assistant.client = new OpenAI(config.openai_api_key);
                ...
                const result = await assistant.run(user_prompt, baseTools.funcs, baseTools.schemas, config.openai_api_key, (event: string, value: any) => { });
