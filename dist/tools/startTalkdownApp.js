...
module.exports = (config) => ({
    schema: {
        type: "function",
        function: {
            name: "start_talkdown_app",
            description: "Start a talkdown app using the optional input data by executing the talkdown entry point specified in the config",
            parameters: {
                type: "object",
                properties: {
                    inputs: {
    ...
    function: async ({ code, inputs = {} }) => {
        const prompt = (schemas) => `You are a Talkdown code executor. Identify Task Type and Name: Clearly specify the type (function, directive, or process) and the name of the task you are executing. For example, "Executing [Type: Function/Directive/Process] - [Name], version [Version Number]."
...
*** NOW LOOK AT THE ./config.json file, RETRIEVE THE ENTRY POINT, AND EXECUTE IT ***
`;
        try {
            ...
            try {
                ...
                const result = await assistant.run(user_prompt, baseTools.funcs, baseTools.schemas, config.openai_api_key, (event, value) => { });
